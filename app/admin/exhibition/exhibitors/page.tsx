...
export default function ExhibitorsPage() {
  ...
  const { stats: exhibitorStats, isLoading: statsLoading } = useExhibitorStats();

  const sectors = ['all', 'Rail', 'Maritime', 'Air', 'Warehouse', 'Technology', 'Logistics', 'Transport', 'Shipping'];
  const statuses = ['all', 'active', 'inactive', 'pending', 'approved', 'rejected'];

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this exhibitor? This action cannot be undone.')) {
      return;
    }
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await updateExhibitor(id, { status: newStatus });
  ...
  const handleBulkStatusUpdate = async (status: string) => {
    const selectedIds = exhibitors.map((e) => e.id);
    if (selectedIds.length === 0) {
      toast.error('No exhibitors selected');
      return;
    }

    try {
      await bulkUpdateStatus(selectedIds, status);
      toast.success(`${selectedIds.length} exhibitors updated to ${status}`);
    } catch (error: any) {
      toast.error(error.message || 'Failed to update exhibitors');
    }
  };
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      await exportExhibitors('csv');
      toast.success('Exhibitors exported successfully');
    } catch (error: any) {
      toast.error(error.message || 'Failed to export exhibitors');
    } finally {
      setIsExporting(false);
    }
  };

  const handleCreateExhibitor = () => {
    router.push('/admin/exhibition/exhibitors/new');
  };

  const handleEditExhibitor = (id: string) => {
    router.push(`/admin/exhibition/exhibitors/edit/${id}`);
  };

  const handleViewExhibitor = (id: string) => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading exhibitors...</p>
        </div>
      </div>
    );
  }
