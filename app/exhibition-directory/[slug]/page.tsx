...
import { fetchExhibitionCompanyById, fetchExhibitorProducts, fetchExhibitorBrochures, fetchExhibitorBrands, ExhibitionCompany, fetchExhibitionCompanies } from '../api';
...
export default function ExhibitorDetailPage() {
    ...
    useEffect(() => {
        const loadCompanyData = async () => {
            ...
            try {
                ...
                // Check if it's a UUID (ID) or a slug (company name)
                const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(slugOrId);

                if (!isUUID) {
                    // It's a slug, find the company by slug first
                    const { companies } = await fetchExhibitionCompanies(1, 1000, '');
                    const foundCompany = companies.find(c =>
                        c.company.toLowerCase().replace(/\s+/g, '-') === slugOrId
                    );

                    if (foundCompany) {
                        actualId = foundCompany.id;
                    } else {
                        throw new Error('Company not found');
                    }
                }

                // Fetch company details with the actual ID
                companyData = await fetchExhibitionCompanyById(actualId);
                setCompany(companyData);

                // Fetch related data in parallel
                const [productsData, brochuresData, brandsData] = await Promise.all([
                    fetchExhibitorProducts(actualId),
                    fetchExhibitorBrochures(actualId),
                    fetchExhibitorBrands(actualId)
                ]);
    ...
    const handleShare = () => {
        if (navigator.share) {
            navigator.share({
                title: company?.company || company?.name,
                text: `Check out ${company?.company || company?.name} at the exhibition`,
    ...
    return (
        <div className="bg-[#F5F8FB] min-h-screen">
            ...
            <div className="pt-16 lg:pt-20 pb-6">
                                    ...
                                    <div className={`w-full max-w-xs mx-auto lg:mx-0 lg:w-64 lg:h-64 rounded-xl border flex items-center justify-center p-8 overflow-hidden ${getLogoColor(company.countryCode)
                                        ...
                                        {company.logo && !imageError ? (
                                            ...
                                            <div className="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-700">
                                                {(company.company || company.name).substring(0, 2).toUpperCase()}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1">
                                    <div className="mb-4">
                                        ...
                                        {company.name && company.name !== company.company && (
                                            <p className="text-gray-500 mt-1 text-sm">Contact Person: {company.name}</p>
                                        )}
                                    </div>
                ...
                {/* CONTENT AREA - Keep the same as before */}
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                    {/* ABOUT TAB */}
                    {activeTab === 'about' && (
                        <div className="space-y-6">
                            <div className="bg-white rounded-2xl p-6 sm:p-8 border shadow-sm">
                                ...
                                <p className="text-gray-700 leading-relaxed text-sm sm:text-base">
                                    {company.description || `${company.company || company.name} is a leading company in the ${company.sector.join(' and ')} sector, 
                                ...
                                products.map((product) => (
                                    <div key={product.id} className="bg-white rounded-2xl p-6 border shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <p className="text-gray-600 text-center text-sm sm:text-base">
                                            {product.description || `${company.company || company.name} services`}
                                        </p>
                                        {product.price && (
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className={`w-14 h-14 sm:w-16 sm:h-16 ${getLogoColor(company.countryCode)} rounded-lg flex items-center justify-center flex-shrink-0`}>
                                                <div className="text-lg font-bold text-gray-600">
                                                    {brand.name?.substring(0, 2).toUpperCase() || (company.company || company.name).substring(0, 2).toUpperCase()}
                                                </div>
                                            </div>
                                            <div className="min-w-0">
                                                <h3 className="text-lg font-semibold text-gray-900 truncate">{brand.name || company.company || company.name}</h3>
                                                {brand.description && (
